...
import { MESSAGE_TYPES, RUNTIME_MODES } from "./runtime-protocol";
...
export default class VersionWatcherWrapper {
  ...
  _validateOptions(options) {
    ...
    if (
      options.checkNowThrottleTime &&
      (typeof options.checkNowThrottleTime !== "number" ||
        options.checkNowThrottleTime < 1000)
    ) {
  ...
  _initRuntime() {
    if (typeof SharedWorker !== "undefined") {
      try {
        this.sharedWorkerManager =
          getSharedWorkerClientManager(SharedVersionWorker);
  ...
  _registerRuntime() {
    if (this.mode === RUNTIME_MODES.SHARED_WORKER && this.sharedWorkerManager) {
      this.sharedWorkerManager.register(
        this.clientId,
        this._createRuntimePayload(),
        (message) => {
  ...
  _handleVersionChange(event, isTip) {
    for (const callback of this.callbacks) {
      try {
        ...
        console.error(
          "[VersionWatcherWrapper] Callback execution failed:",
          error,
        );
      }
    }
